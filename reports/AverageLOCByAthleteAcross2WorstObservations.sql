SELECT Athlete.IDAThlete,
       ...
       Athlete.LastName,
       Athlete.FirstName
FROM (SELECT IDAthlete, BibNumber, LOCAverage, RANK() OVER (Partition by BibNumber Order by LOCAverage DESC) LOCRank
...
WHERE LOCData.LOCRank < 3
  AND LOCData.LOCAverage >= 50
GROUP BY LOCData.BibNumber
HAVING LOCData.LOCAverage >= 50
   AND COUNT(LOCData.BibNumber) >= 2
