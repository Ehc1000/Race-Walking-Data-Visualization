SELECT FinalXRedLOC.LastName,
       FinalXRedLOC.FirstName,
       IFNULL(FinalXRedLOC.XRedLOC, 0)              AS XRedLOC,
       IFNULL(FinalMatchLOC.CorRedLOC, 0)           AS CorRedLOC,
       IFNULL(FinalXRedBent.XRedBent, 0)            AS XRedBent,
       IFNULL(FinalMatchBent.CorRedBent, 0)         AS CorRedBent,
...
---Judges Matching the LOC Video Observations
    (SELECT J.FirstName, J.LastName, J.IDJudge, COUNT(BibNumber) AS CorRedLOC
     FROM Judge J
              INNER JOIN
...
---Judges Matching the Bent Knee Video Observations
    (SELECT J.FirstName, J.LastName, J.IDJudge, COUNT(BibNumber) AS CorRedBent
     FROM Judge J
              INNER JOIN
          (SELECT JC.IDJudge, JC.BibNumber
           FROM JudgeCall JC
