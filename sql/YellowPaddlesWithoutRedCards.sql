Select JudgeSelect.FirstName, JudgeSelect.LastName,
max(case WHEN Infraction = '~' then InfractionCount else 0  end) LOC,
max(case WHEN Infraction = '<' then InfractionCount else 0  end) Bent FROM
(SELECT Judge.FirstName, Judge.LastName, Judge.IDJudge FROM Judge INNER JOIN RaceJudge ON Judge.IDJudge = RaceJudge.IDJudge WHERE RaceJudge.IDRace=6) JudgeSelect 
...
FROM JudgeCall INNER JOIN Bib ON (Bib.BibNumber = JudgeCall.BibNumber AND Bib.IDRace = JudgeCall.IDRace)
...
HAVING Count(JudgeCall.BibNumber) = 1 AND MAX(JudgeCall.Color)='Yellow') RedCardSub
