SELECT FinalExcessive.LastName,
       FinalExcessive.FirstName,
       IFNULL(FinalExcessive.ExcessiveRedCards, 0)  AS ExcessiveRedCards,
       IFNULL(FinalMatching.CorrectRedCards, 0)     AS CorrectRedCards,
       IFNULL(FinalMissing.MissedLOCRed, 0)         AS MissedLOCRed,
       IFNULL(FinalMissing.MissedBentRed, 0)        AS MissedBentRed,
       ...
       IFNULL(FinalMatching.CorrectRedCards, 0) * 2 + IFNULL(FinalNoCalls.NbrCorrectNonCalls, 0) * .5 -
       IFNULL(FinalExcessive.ExcessiveRedCards, 0) - IFNULL(FinalMissing.MissedLOCRed, 0) * 2 -
       IFNULL(FinalMissing.MissedBentRed, 0) * 2 -
...
(SELECT Judge.FirstName, Judge.LastName, JC.IDJudge, COUNT(JC.BibNumber) AS ExcessiveRedCards
 FROM JudgeCall JC
          INNER JOIN Judge ON JC.IDJudge = Judge.IDJudge
 WHERE JC.Color = "Red"
...
---Judges Matching the Video Observations
    (SELECT J.FirstName, J.LastName, J.IDJudge, COUNT(BibNumber) AS CorrectRedCards
     FROM Judge J
              INNER JOIN
          (SELECT JC.IDJudge, JC.BibNumber
           FROM JudgeCall JC
    ...
    (SELECT Judge.FirstName,
     ...
     FROM (SELECT IDJudge, COUNT(BibNumber) AS MissedNumber, Color, Infraction
           FROM (SELECT RaceJudge.IDJudge, CorrectCalls.BibNumber, CorrectCalls.Color, CorrectCalls.Infraction
                 FROM (SELECT VO.BibNumber, VO.IDRace, "Red" AS Color, "<" AS Infraction
                       FROM VideoObservation VO
                       WHERE KneeAngle > 0
                         AND KneeAngle <= 175
                         AND VO.IDRace = :race_id
                       ...
                       HAVING COUNT(VO.BibNumber) > 1
                       UNION
                       SELECT BibNumber, VO.IDRace, "Red" AS Color, "~" AS Infraction
                       FROM VideoObservation VO
                       WHERE VO.LOCAverage >= 55
                         AND VO.IDRace = :race_id
                       GROUP BY VO.BibNumber
                       HAVING COUNT(VO.BibNumber) > 1) CorrectCalls,
                      RaceJudge
                 WHERE RaceJudge.IDRace = :race_id
                 EXCEPT
                 SELECT IDJudge, BibNumber, Color, Infraction
                 FROM JudgeCall
...
---Judges Discrepancy from the Video Observations 
    (SELECT Judge.IDJudge, COUNT(Discrepancies.BibNumber) AS NbrDiscrepancies, Judge.FirstName, Judge.LastName
     FROM (SELECT ActualJudgeCalls.IDJudge, ActualJudgeCalls.BibNumber
           FROM (SELECT RaceJudge.IDJudge, CorrectCalls.BibNumber, CorrectCalls.Color, CorrectCalls.Infraction
                 FROM (SELECT VO.BibNumber, VO.IDRace, "Red" AS Color, "<" AS Infraction
                       FROM VideoObservation VO
                       WHERE KneeAngle > 0
                       ...
                       HAVING COUNT(VO.BibNumber) > 1) CorrectCalls,
                      RaceJudge
                 WHERE RaceJudge.IDRace = :race_id) VideoCalls
                    INNER JOIN
                (SELECT IDJudge, BibNumber, Color, Infraction
                 FROM JudgeCall
                 WHERE IDRace = :race_id AND Color = "Red") ActualJudgeCalls
                ON VideoCalls.BibNumber = ActualJudgeCalls.BibNumber
           WHERE ActualJudgeCalls.Infraction <> VideoCalls.Infraction
           GROUP BY ActualJudgeCalls.IDJudge, ActualJudgeCalls.BibNumber) Discrepancies
              INNER JOIN Judge ON Discrepancies.IDJudge = Judge.IDJudge
...
-- Judge Non calls from the Video Observations
    (SELECT J.FirstName, J.LastName, AllJudgesAllBib.IDJudge, Count(AllJudgesAllBib.BibNumber) AS NbrCorrectNonCalls
     FROM (SELECT BibNumber, IDJudge
           FROM Bib,
                RaceJudge
           WHERE Bib.IDRace = :race_id
             AND RaceJudge.IDRace = :race_id) AllJudgesAllBib
              INNER JOIN Judge J ON J.IDJudge = AllJudgesAllBib.IDJudge
              LEFT JOIN
          (SELECT BibNumber, IDJudge FROM JudgeCall WHERE IDRace = :race_id AND Color = "Red") JudgeCalls
          ON AllJudgesAllBib.IDJudge = JudgeCalls.IDJudge AND AllJudgesAllBib.BibNumber = JudgeCalls.BibNumber
     WHERE JudgeCalls.IDJudge IS NULL
       AND AllJudgesAllBib.BibNumber IN
---Bibs of legal walkers in a race 
           (SELECT BibNumber
            FROM BIB
            WHERE IDRace = :race_id
              AND BibNumber NOT IN (SELECT VO.BibNumber
                                    FROM VideoObservation VO
                                    WHERE KneeAngle > 0
                                      AND KneeAngle <= 175
                                      AND VO.IDRace = :race_id
