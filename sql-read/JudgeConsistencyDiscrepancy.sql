...
FROM (SELECT JudgeCall.IDJudge, Count(JudgeCall.BibNumber) AS MajorityNumber, JudgeCall.Infraction, JudgeCall.Color
      FROM JudgeCall
               LEFT JOIN
           (SELECT JudgeCall.BibNumber, JudgeCall.IDRace, JudgeCall.Color, JudgeCall.Infraction
            ...
            HAVING (Count(JudgeCall.Infraction)) >=
                   (Select count(idJudge) / 2 from RaceJudge WHERE RaceJudge.IDRace = :race_id)) MajorityCallPerAthlete
           ON (JudgeCall.Infraction = MajorityCallPerAthlete.Infraction) AND
              (JudgeCall.Color = MajorityCallPerAthlete.Color) AND
              (JudgeCall.BibNumber = MajorityCallPerAthlete.BibNumber) AND
              (JudgeCall.IDRace = MajorityCallPerAthlete.IDRace)
      WHERE (MajorityCallPerAthlete.BibNumber IS NULL)
